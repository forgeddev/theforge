module.exports = function(app){
  ...
  *****************************************************************************/
  ...
  app.get('/programReferral/cards', [routeSecurity.userAuthenticated], function(req, res){

    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral')
      , filterModel = {
    ...
    req.session.programReferral = {
      "back_page": "/patient/view/"+req.session.selected_patient.recordId
    };

    programReferral.find( filterModel, sortModel, orderModel).then( function(findResponse){
      mountainComplex.buildPageResponse(req, [findResponse]).then(function(response){
        res.render('mc/programReferral/cards', response);
      });
  ...
  app.get('/programReferral/edit/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      programReferral.findById({ "recordId": req.params.id }).then(function( findResponse){
        if( findResponse
          && findResponse.data
          ...
          // save in session
          req.session.selected_programReferral = findResponse.data.rows[0];
          ...
          mountainComplex.buildPageResponse(req, [
            ...
            res.render('mc/programReferral/form', response);
          });

        } else {
          req.session.selected_programReferral = undefined;
  ...
  app.get('/programReferral/view/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral');
      ...
      programReferral.findById({ "recordId": req.params.id }).then(function( findResponse){
        if( findResponse
          ...
          mountainComplex.buildPageResponse(req, [
            ...
            res.render('mc/programReferral/view', response);
          });
  ...
  *****************************************************************************/
  ...
  app.post('/programReferral/save', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral');

    programReferral.save( req.body).then(function( saveResponse){
      req.session.selected_programReferral = saveResponse.reference.model;
  ...
  app.post('/programReferral/remove/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      programReferral.delete({"recordId": req.params.id}).then(function( deleteResponse){
        req.session.selected_programReferral = undefined;
  ...
  app.post('/programReferral/restore/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , programReferral = mountainComplex.getModel('programReferral');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      programReferral.restore({"recordId": req.params.id}).then(function( deleteResponse){
        req.session.selected_programReferral = undefined;
        res.json(  mountainComplex.buildApiResponse( req, [deleteResponse]));
      }).catch(function( error) {
        req.session.selected_programReferral = undefined;
