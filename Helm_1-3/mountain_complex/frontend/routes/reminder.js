module.exports = function(app){
  ...
  *****************************************************************************/
  ...
  app.get('/reminder/cards', [routeSecurity.userAuthenticated], function(req, res){
    ...
    reminder.find( filterModel, sortModel, orderModel).then( function(findResponse){
      mountainComplex.buildPageResponse(req, [findResponse]).then(function(response){
        res.render('mc/reminder/cards', response);
      });
  ...
  app.get('/reminder/edit/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , reminder = mountainComplex.getModel('reminder');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      reminder.findById({ "recordId": req.params.id }).then(function( findResponse){
        if( findResponse
          && findResponse.data
          ...
          // save in session
          req.session.selected_reminder = findResponse.data.rows[0];
          ...
          mountainComplex.buildPageResponse(req, [
            ...
            res.render('mc/reminder/form', response);
          });

        } else {
          req.session.selected_reminder = undefined;
  ...
  app.get('/reminder/view/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , reminder = mountainComplex.getModel('reminder');
      ...
      reminder.findById({ "recordId": req.params.id }).then(function( findResponse){
        if( findResponse
          ...
          mountainComplex.buildPageResponse(req, [
            ...
            res.render('mc/reminder/view', response);
          });
  ...
  *****************************************************************************/
  ...
  app.post('/reminder/save', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , reminder = mountainComplex.getModel('reminder');

    reminder.save( req.body).then(function( saveResponse){
      req.session.selected_reminder = saveResponse.reference.model;
  ...
  app.post('/reminder/remove/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , reminder = mountainComplex.getModel('reminder');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      reminder.delete({"recordId": req.params.id}).then(function( deleteResponse){
        req.session.selected_reminder = undefined;
  ...
  app.post('/reminder/restore/:id?', [routeSecurity.userAuthenticated], function(req, res){
    var mountainComplex = req.app.locals.settings.mountainComplex
      , reminder = mountainComplex.getModel('reminder');

    if( req.params.id && req.params.id > 0 && req.params.id != "new") {
      reminder.restore({"recordId": req.params.id}).then(function( deleteResponse){
        req.session.selected_reminder = undefined;
        res.json(  mountainComplex.buildApiResponse( req, [deleteResponse]));
      }).catch(function( error) {
        req.session.selected_reminder = undefined;
