DROP TABLE IF EXISTS `programReferral`;
CREATE TABLE `programReferral` (
  ...
  , `summary` VARCHAR(256)
  , `notes` VARCHAR(2000)
  , `referralSourceType` BIGINT
  , `referralOutcomeType` BIGINT
  , `when` DATETIME
  , `duration` INTEGER
  , `name` VARCHAR(256)
  ...
  , FOREIGN KEY fk_referralSourceType(referralSourceType) REFERENCES list(recordId) ON UPDATE CASCADE ON DELETE CASCADE
  , FOREIGN KEY fk_referralOutcomeType(referralOutcomeType) REFERENCES list(recordId) ON UPDATE CASCADE ON DELETE CASCADE
